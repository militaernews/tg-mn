...
from data.db import get_patterns, get_footer
...
def format_text(text: str, message: Message, source: SourceDisplay) -> str:
    if message.forward_from_chat.username is not None:
        username = f"https://t.me/{message.forward_from_chat.username}/"
    else:
        username = f"tg://privatepost?channel={message.forward_from_chat.id}&post="

    formatted = f"{text}\n\nQuelle: <a href='{username}{message.forward_from_message_id}'>{source.display_name}"
    if source.bias is not None:
        formatted += f" {source.bias}"
    formatted += f"</a> |<a href='https://t.me/nn_backup/{message.id}'> 💾 </a>"
